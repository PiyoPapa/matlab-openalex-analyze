...
診断用の例：同一の UMAP 投影に対して、異なるクラスタリングの仮定で色付けしたもの。
左：密度に基づくグルーピング（HDBSCAN）で、局所的な集中とノイズを強調している。
右：固定分割のグルーピング（k-means）で、全体を強制的に分割している。
クラスタの色や境界は、検証されたトピックではなく、分析的な意味を持たない。
